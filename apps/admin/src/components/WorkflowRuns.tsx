 ...
 * ArticleWorkflow runs.
 *
 * Each run renders as a card: a chevron strip of pipeline stages (fetch →
 * mirror → transcribe → translate → localize), an overall progress bar, and —
 * for the multi-image stages — a stacked per-image list with a spinner on
 * whichever images are mid-flight. All stage states are derived client-side
 * from the run's StateSnapshot + per-image detail; the wire stays
 * machine-readable.
 */
...
function RunCard({ run }: { run: WorkflowRunEntry }) {
