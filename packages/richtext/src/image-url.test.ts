...
describe('rewriteImageSrc', () => {
  it('proxies the CDN host, preserving the host in the path', () => {
    expect(
      rewriteImageSrc('https://cache.hiroba.dqx.jp/dq_resource/imgs/a.jpg'),
    ).toBe('/img/cache.hiroba.dqx.jp/dq_resource/imgs/a.jpg');
    expect(rewriteImageSrc('http://cache.hiroba.dqx.jp/x/y.png')).toBe(
      '/img/cache.hiroba.dqx.jp/x/y.png',
    );
  });
  ...
  it('canonicalizes CDN aliases to the primary host (dedup)', () => {
    ...
    expect(
      rewriteImageSrc('https://close.cache.hiroba.dqx.jp/dq_resource/a.jpg'),
    ).toBe('/img/cache.hiroba.dqx.jp/dq_resource/a.jpg');
  });
  ...
  it('keeps distinct DQX hosts distinct (no collision)', () => {
    ...
    expect(
      rewriteImageSrc('https://faceicon.dqx.jp/icon1/846/8460/x.jpg'),
    ).toBe('/img/faceicon.dqx.jp/icon1/846/8460/x.jpg');
  ...
  it('drops query strings (not part of the R2 key)', () => {
    ...
    expect(rewriteImageSrc('https://cache.hiroba.dqx.jp/a.jpg?v=2')).toBe(
      '/img/cache.hiroba.dqx.jp/a.jpg',
    );
  });
  ...
  it('accepts a custom base for serving straight from a bucket domain', () => {
    ...
    ).toBe('https://img.example.com/cache.hiroba.dqx.jp/a.jpg');
    expect(
      rewriteImageSrc('/dq_resource/a.jpg', 'https://img.example.com'),
    ).toBe('https://img.example.com/cache.hiroba.dqx.jp/dq_resource/a.jpg');
  });

  it('proxies protocol-relative CDN URLs', () => {
    expect(rewriteImageSrc('//cache.hiroba.dqx.jp/a.jpg')).toBe(
      '/img/cache.hiroba.dqx.jp/a.jpg',
    );
  });

  it('proxies root-relative dq_resource paths to the canonical host', () => {
    expect(rewriteImageSrc('/dq_resource/imgs/a.jpg')).toBe(
  ...
  it('leaves off-site (non-dqx.jp) URLs untouched', () => {
    ...
    ).toBe('http://www.ganganonline.com/comic/dqx/img/comic_main.jpg');
    expect(rewriteImageSrc('https://i.ytimg.com/vi/x/hqdefault.jpg')).toBe(
  ...
  it('leaves other relative paths and data URIs untouched', () => {
    ...
    expect(rewriteImageSrc('data:image/png;base64,AAAA')).toBe(
...
describe('imageKey', () => {
  it('keys mirrorable DQX images (query-less), canonicalizing aliases', () => {
    expect(imageKey('https://cache.hiroba.dqx.jp/dq_resource/a.jpg')).toBe(
      'cache.hiroba.dqx.jp/dq_resource/a.jpg',
    );
    expect(imageKey('https://hiroba.dqx.jp/dq_resource/a.jpg')).toBe(
      'cache.hiroba.dqx.jp/dq_resource/a.jpg',
    );
    expect(imageKey('/dq_resource/a.jpg?v=2')).toBe(
      'cache.hiroba.dqx.jp/dq_resource/a.jpg',
    );
    expect(imageKey('https://faceicon.dqx.jp/i/x.jpg')).toBe(
      'faceicon.dqx.jp/i/x.jpg',
    );
  });
  ...
  it('round-trips to the upstream URL', () => {
    ...
    expect(imageUpstreamUrl(key!)).toBe(
      'https://cache.hiroba.dqx.jp/dq_resource/a.jpg',
    );
  });
});
      ...
      },
      {
        ...
        children: [
          ...
          { type: 'icon', src: '/dq_resource/ico_2nd.gif' },
          {
            type: 'link',
            href: '#',
            children: [{ type: 'icon', src: '/dq_resource/ico_3rd.gif' }],
          },
        ],
      },
      {
