...
/**
 * The R2 key of an image object's AVIF re-encode: the base key plus `.avif`,
 * so the variant sits next to its source and the URL's final extension is
 * always truthful. Appending (not swapping) keeps derivation collision-free
 * for mirrored originals (`foo.jpg` and `foo.png` may both exist upstream).
 * Existence is NOT implied — encoding is skipped/failed for some sources
 * (animated GIFs, oddball formats), so readers consult the recorded
 * `image_sources` rows, never derive blindly.
 */
export const avifVariantKey = (key: string): string => `${key}.avif`;

/** A "resize to fit inside width×height" box (never enlarges). */
export type FitSize = { width: number; height: number };

/**
 * The R2 key of a resized variant of an image object — scaled down to fit
 * inside `size`, re-encoded as `contentType`: `<key>.fit<W>x<H><ext>`. Named
 ...
 * dimensions live on the variant's image_sources row. Same appending rules
 * as avifVariantKey: collision-free beside the primary, extension truthful.
 */
